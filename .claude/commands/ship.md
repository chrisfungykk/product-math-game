...
`/ship` is a **fan-out orchestrator**. It runs six specialist personas in parallel against the current change, then merges their reports into a single go/no-go decision with a rollback plan. The personas operate independently — no shared state, no ordering — which is what makes parallel execution safe and useful here.

## Phase A — Parallel fan-out

Spawn six subagents concurrently using the Agent tool. **Issue all six Agent tool calls in a single assistant turn so they execute in parallel** — sequential calls defeat the purpose of this command.
...
4. **`software-architect`** — Review architecture, module boundaries, dependency direction, maintainability, and release risk. Output the standard architecture report.
5. **`frontend-engineer`** — Review Next.js/React UI quality, accessibility, state, design-token fidelity, and browser verification. Output the standard frontend report.
6. **`backend-engineer`** — Review API contracts, validation, persistence, data integrity, error handling, and operational impact. Output the standard backend report.
...
3. **Tests** — Promote failing or missing critical coverage from `test-engineer`; cross-check with code quality and backend/frontend risks.
4. **Architecture** — Promote boundary, dependency, and release-risk blockers from `software-architect`.
5. **Frontend** — Promote accessibility, React state, UI fidelity, and browser-verification blockers from `frontend-engineer`.
6. **Backend** — Promote contract, validation, persistence, migration, and operational blockers from `backend-engineer`.
7. **Performance** — Pull from `code-reviewer`, `frontend-engineer`, and `backend-engineer`; cross-check Core Web Vitals if applicable.
8. **Infrastructure** — Env vars, migrations, monitoring, feature flags. Verify directly.
9. **Documentation** — README, ADRs, changelog. Verify directly.
...
1. The six Phase A personas run in parallel — never sequentially.
